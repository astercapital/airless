...
class TestEmailHook(unittest.TestCase):
    ...
    def test_build_message_with_attachments(self):
        ...
        message = self.email_hook.build_message(subject, content, recipients, sender, attachments)
        ...
        self.assertEqual(len(message.get_payload()), 2)  # Ensure two attachments are added

        # Check the first attachment
        part1 = message.get_payload()[0]  # The first payload will be the text part
        self.assertIsInstance(part1, MIMEBase)
        self.assertEqual(part1.get_filename(), 'test.txt')

        # Check the second attachment
        part2 = message.get_payload()[1] 
    ...
    def test_send_method_not_implemented(self):
        ...
        with self.assertRaises(NotImplementedError):
            self.email_hook.send("subject", "content", ["to@example.com"], "from@example.com", [], "plain")
